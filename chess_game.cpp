...
int main()
{
    ...
    printf("Enter 1 for unlimited move and 2 for 50 moves: ");
    scanf("%d",&a);
    ...
    for(float i=0; i<=6; i=i+0.1)
    {
        ...
        outtextxy(50,120,"WELCOME TO THE CHESS"); // font size increases
        delay(30);
        cleardevice();
    }
    outtextxy(50,120,"WELCOME TO THE CHESS");    // the final part where it shows welcome to the chess
    settextstyle(1,HORIZ_DIR,3);
    getch();
    cleardevice();

    //Displaying Group Members and Roll Numbers
    settextstyle(10,HORIZ_DIR,15);
    setcolor(RED);
    outtextxy(50,200,"Designed by:");
    setcolor(1);
    outtextxy(50,250,"Ahmed Azaz Humdoon");
    outtextxy(350,250,"170041032");
    outtextxy(50,220,"Refaat Mohammed   ");
    outtextxy(350,220,"170041031");
    ...
    //Displaying The Works To Be Done
	setcolor(6);
...
\b\b\b\b\bEnter File name to Load \n";
        ...
        if(ch==27)   //for escaping
        {
            break;
        }
        else if(ch=='u'||ch=='U')  //for undo work
        {
            x=-1;
            x1=-1;
            y1=-1;
        ...
        else if(y==-1)
        {
            if(ch>='1' && ch<='8')
            {
                ...
                else
                {
                    ...
                    {
                        if(board[y][x]>0)
                            //if player 2 has selected a piece of player 1
                        {
                            wrongStart();
                            x=-1;
                            y=-1;
...
\bwhich piece Q,R,H,B";
    ...
    printf("game duration is %f seconds\n",time_taken);
...
void printboard()
{
    for(int i=0; i<8; i++)
    {
        for(int j=0; j<8; j++)
        {
            ...
            outtextxy(270+(j*45),105+(i*45),"UU");
...
int bishop(int x,int y,int x1,int y1)
{
    ...
    if((x<x1)&&(y<y1))     //moves to the upper right hand side diagonal
    {
        for(i=1; (i+x)<x1; i++)
        {
            if(board[y+i][x+i]!=0)
    ...
    else if((x>x1)&&(y>y1))    //moves to the lower left hand side diagonal
    {
        for(i=1; (x-i)>x1; i++)
        {
            if(board[y-i][x-i]!=0)
    ...
    else if((x>x1)&&(y<y1))    // moves to the upper left hand side diagonal
    {
        for(i=1; (x-i)>x1; i++)
        {
            if(board[y+i][x-i]!=0)
    ...
    else if((x<x1)&&(y>y1))    //moves to the lower right hand side diagonal
    {
        for(i=1; (y-i)>y1; i++)
        {
            if(board[y-i][x+i]!=0)
...
int rook(int x,int y,int x1,int y1)
{
    ...
    if(y1==y)
    {
        for(int i=x+1; i<x1; i++)   //goes to upper row on the same column (5,5) to (5,8)
        {
            if(board[y1][i]!=0)
            {
                a = 0;
        ...
        for(int i=x-1; i>x1; i--) //goes to lower row on the same column (5,5) to (5,0)
        {
            if(board[y1][i]!=0)
            {
                a = 0;
    ...
    {
        for(int i=y+1; i<y1; i++) //goes to the right column in the same row (5,5) to (8,5)
        {
            if(board[i][x1]!=0)
            {
                a = 0;
        ...
        for(int i=y-1; i>y1; i--) //goes to the left column in the same row (5,5) to (0,5)
        {
            if(board[i][x1]!=0)
            {
                a = 0;
